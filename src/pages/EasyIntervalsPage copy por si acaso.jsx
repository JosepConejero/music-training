...
export const EasyIntervalsPage = () => {
  ...
  /* const isDoShowed = useRef(true);
  const isDosShowed = useRef(false);*/ 
  /* const isDoBlocked = useRef(true);
  const isDosBlocked = useRef(false);  */
  ...
  const updateisDosShowedAction = ()=>{
   // isDosShowed.current = !isDosShowed.current;
    console.log(isDosShowed.current);
    //if isDosShowed
    if (filteredNotesRef.current.length > 1) {
...
/*     if (isDoShowed) {
      ...
      if (index !== -1) {       if (filteredNotesRef.current.length > 1) { filteredNotesRef.current.splice(index, 1);}
      else {
        setIsDoShowed(true);

      }
       //updateIsDoShowed(false);
      } else {
      ...
      console.log(filteredNotesRef.current);
      //filteredNotesRef.current = [...filteredNotesRef.current, {noteUp: "do", noteDown: "DO"}];
   ...
   // if (filteredNotesRef.current.length===0) console.log(filteredNotesRef.current.length);
